...
const AdminSubcategories = () => {
    ...
    useEffect(() => {
        const fetchSubcategories = async () => {
            ...
            try {
                ...
                const res = await axiosInstance.get(`/api/subcategories?categoryId=${categoryId}`);
                ...
                setApiError(`API Error: ${error.response?.status || 'Unknown'} - ${error.response?.data?.error || error.message}`);

                // Try alternative API endpoint
                try {
                    console.log("Trying alternative API endpoint...");
                    const altRes = await axiosInstance.get(`/api/subcategories?category=${categoryId}`);
                    ...
                    try {
                        ...
                        const filteredSubcategories = allRes.data.subcategories?.filter((sub: any) => sub.categoryId === categoryId) || [];
    ...
    const updateSubcategory = async (subcategoryId: string, subcategory: any) => {
        setLoading(true);
        try {
            await axiosInstance.put(`/api/subcategories/${subcategoryId}`, subcategory);
            await fetchSubcategories();
        } finally {
            setLoading(false);
        }
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {subcategories.map((subcategory) => (
                        ...
                        >
                            <h3 className="font-bold text-lg text-green-700">{subcategory.name}</h3>
                            ...
                            <div className="flex gap-2 mt-2">
                                ...
                                <button
                                    ...
                                    onClick={() => navigate(`${location.pathname}/${subcategory._id}`)}
                                >
                                    View Details
                                </button>
                            </div>
